...
function M.cmp_source(name, icon)
  ...
  local colors = {
    ok = CangsVim.ui.fg("Special"),
    error = CangsVim.ui.fg("DiagnosticError"),
    pending = CangsVim.ui.fg("DiagnosticWarn"),
  }

  return {
    function()
      return icon or CangsVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
    end,
    cond = function()
      return status() ~= nil
    end,
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = CangsVim.root.get({ normalize = true })
    local cwd = CangsVim.root.cwd()
