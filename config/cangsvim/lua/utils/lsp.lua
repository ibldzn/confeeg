...
function M.rename_file()
  ...
  local old = assert(CangsVim.root.realpath(vim.api.nvim_buf_get_name(buf)))
  local root = assert(CangsVim.root.realpath(CangsVim.root.get({ normalize = true })))
...
---@param opts? CangsFormatter| {filter?: (string|lsp.Client.filter)}
function M.formatter(opts)
  ...
  local ret = {
    ...
    priority = 1,
    format = function(buf)
      M.format(CangsVim.merge({}, filter, { bufnr = buf }))
    end,
    sources = function(buf)
      local clients = M.get_clients(CangsVim.merge({}, filter, { bufnr = buf }))
      ---@param client vim.lsp.Client
  ...
  return CangsVim.merge(ret, opts) --[[@as CangsFormatter]]
end
...
function M.format(opts)
  opts = vim.tbl_deep_extend(
    ...
    {},
    opts or {},
    CangsVim.opts("nvim-lspconfig").format or {},
    CangsVim.opts("conform.nvim").format or {}
  )
  ...
  M.on_supports_method("textDocument/documentHighlight", function(_, buf)
    vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
      ...
      callback = function(ev)
        if not require("plugins.lsp.mappings").has(buf, "documentHighlight") then
          return false
        end
        ...
        if not ({ M.words.get() })[2] then
          if ev.event:find("CursorMoved") then
            vim.lsp.buf.clear_references()
          elseif not CangsVim.cmp.visible() then
